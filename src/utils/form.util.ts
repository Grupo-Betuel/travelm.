import formHelperStyles from '@customer-service/styles/forms-helpers.module.scss';
import { FieldErrors, RegisterOptions } from "react-hook-form";

export type IFieldValidatorStructure = (criteria?: Partial<RegisterOptions>) => {
    className: string;
}
...
export function getFieldValidatorClassName(type: FieldValidatorTypes, criteria?: RegisterOptions) {
...
export default function getFormValidatorClassNames<T>(entity: FormValidatorEntityType<T>, formError: FieldErrors<T>): FormValidatorEntityClassNamesType<T> {
    ...
    Object.keys(entity).forEach((entityFieldName: string) => {
        ...
        Object.keys(fieldValidation).forEach((optionValidatorKey: any) => {
            ...
            if (fieldError && fieldError.type === optionValidatorKey) {
                entityValidationClassNames.push(getFieldValidatorClassName(optionValidatorKey, fieldValidation))
            }

        });

        validatorEntityClassNames[entityFieldName] = entityValidationClassNames.join(' ');
    });
